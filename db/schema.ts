...
export const conversations = sqliteTable("conversations", { id: text("id").primaryKey(), conversationId: text("conversation_id"), agentId: text("agent_id").notNull(), role: text("role").notNull(), message: text("message").notNull(), status: text("status").notNull().default("ready"), createdAt: text("created_at").notNull() });
...
export const integrationConnections = sqliteTable("integration_connections", { id: text("id").primaryKey(), provider: text("provider").notNull().unique(), accountLabel: text("account_label").notNull(), encryptedToken: text("encrypted_token").notNull(), scopes: text("scopes").notNull(), status: text("status").notNull(), connectedAt: text("connected_at").notNull(), lastChecked: text("last_checked") });
export const oauthStates = sqliteTable("oauth_states", { id: text("id").primaryKey(), provider: text("provider").notNull(), stateHash: text("state_hash").notNull().unique(), accountLabel: text("account_label").notNull(), expiresAt: text("expires_at").notNull(), usedAt: text("used_at") });
