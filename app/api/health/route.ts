...
import { verifyCalendarConnection } from "../../../lib/calendar";
...
export async function GET() {
  ...
  const [shopify, gmail, meta, ga4, posthog, scheduling, redis] = await Promise.all([verifyShopify(), verifyGmailConnection(), verifyMetaConnection(), verifyGa4Connection(), verifyPostHogConnection(), verifyCalendarConnection(), verifyRedis()]);
  const integrations = {
    openai: ai,
    shopify,
    gmail,
