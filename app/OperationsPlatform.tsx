...
function MediaPanel({ contextType, contextId, attachments, generations = [], onChange, compact = false }: any) {
  const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [creating, setCreating] = useState(false); const [prompt, setPrompt] = useState(""); const [kind, setKind] = useState("image");
  ...
  async function upload(event: React.ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; setBusy(true); setError(""); const form = new FormData(); form.append("file", file); form.append("contextType", contextType); form.append("contextId", contextId); try { const response = await fetch("/api/media", { method: "POST", body: form }); const body = await response.json(); if (!response.ok) throw new Error(body.error || "Upload failed"); await onChange(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Upload failed"); } finally { setBusy(false); event.target.value = ""; } }
  async function generate(event: FormEvent) { event.preventDefault(); if (!prompt.trim()) return; setCreating(true); setError(""); try { const response = await fetch("/api/media/generate", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contextType, contextId, kind, prompt }) }); const body = await response.json(); if (!response.ok) throw new Error(body.error || "Generation failed"); setPrompt(""); await onChange(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Generation failed"); } finally { setCreating(false); } }
  async function refreshJob(id: string) { setCreating(true); setError(""); try { const response = await fetch("/api/media/generate", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "refresh", id }) }); const body = await response.json(); if (!response.ok) throw new Error(body.error || "Status check failed"); await onChange(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Status check failed"); } finally { setCreating(false); } }
  return <div className={`media-panel ${compact ? "compact" : ""}`}><div className="media-actions"><strong>Media</strong><div><label className={busy ? "disabled" : ""}><input type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime" onChange={upload} disabled={busy} /><span>{busy ? "Uploading…" : "Upload"}</span></label><button type="button" onClick={() => setCreating(!creating)}>{creating ? "Close" : "Create with AI"}</button></div></div>{creating && <form className="generation-form" onSubmit={generate}><select aria-label="Media type" value={kind} onChange={(event) => setKind(event.target.value)}><option value="image">Image · GPT Image 2</option><option value="video">Video · Sora 2</option></select><textarea required aria-label="Generation prompt" placeholder="Describe the scene, product, styling, lighting, and composition…" value={prompt} onChange={(event) => setPrompt(event.target.value)} /><button disabled={!prompt.trim() || busy}>{kind === "video" ? "Start video" : "Create image"}</button></form>}{jobs.map((job: any) => <div className="generation-job" key={job.id}><span>{job.status === "failed" ? "Video failed" : `Video ${job.status.replace("_", " ")} · ${job.progress || 0}%`}</span>{job.status !== "failed" && <button type="button" disabled={creating} onClick={() => refreshJob(job.id)}>Check status</button>}</div>)}{error && <p className="form-error" role="alert">{error}</p>}{items.length > 0 && <div className="media-grid">{items.map((item: any) => <a key={item.id} href={`/api/media?id=${encodeURIComponent(item.id)}`} target="_blank" rel="noreferrer" className="media-item">{item.mime_type.startsWith("image/") ? <img src={`/api/media?id=${encodeURIComponent(item.id)}`} alt={item.file_name} loading="lazy" /> : <video src={`/api/media?id=${encodeURIComponent(item.id)}`} controls preload="metadata" aria-label={item.file_name} />}<span>{item.file_name}</span><small>{item.source === "generated" ? "AI-created" : "Uploaded"}</small></a>)}</div>}</div>;
}
