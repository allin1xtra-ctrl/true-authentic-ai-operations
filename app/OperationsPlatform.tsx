...
function Settings({ integrations, health, notifications, setNotifications }: any) {
  const [connectionMessage, setConnectionMessage] = useState(() => {
    ...
    const gmailResult = typeof window === "undefined" ? null : new URLSearchParams(window.location.search).get("gmail");
    const calendarResult = typeof window === "undefined" ? null : new URLSearchParams(window.location.search).get("calendar");
    if (gmailResult) return gmailResult === "connected" ? "Gmail connected and verified with read-only access." : gmailResult === "denied" ? "Gmail authorization was cancelled." : gmailResult === "permissions" ? "Google did not grant the required Gmail read-only scope." : gmailResult === "invalid" ? "The Gmail authorization request expired. Please try again." : "Gmail authorization could not be verified. Please try again.";
    if (calendarResult) return calendarResult === "connected" ? "Google Calendar connected and verified with read-only access." : calendarResult === "denied" ? "Google Calendar authorization was cancelled." : calendarResult === "permissions" ? "Google did not grant the required Calendar read-only scopes." : calendarResult === "invalid" ? "The Calendar authorization request expired. Please try again." : "Google Calendar authorization could not be verified. Please try again.";
  ...
  const [connectingGoogle, setConnectingGoogle] = useState<"gmail" | "calendar" | null>(null);
  const setupDetails = {
    gmail: { title: "Connect Gmail", text: "Enable the Gmail API and register the OAuth callback displayed on the Gmail card. Only gmail.readonly is requested; sending remains disabled.", href: "https://console.cloud.google.com/apis/credentials", label: "Open Google Cloud" },
    ...
    calendar: { title: "Connect Google Calendar", text: "Enable the Calendar API and register the OAuth callback displayed on the Calendar card. Only calendar-list and event read access is requested.", href: "https://console.cloud.google.com/apis/credentials", label: "Open Google Cloud" },
    ga4: { title: "Connect Google Analytics 4", text: "Register the OAuth callback displayed on the GA4 card. Only analytics.readonly is requested.", href: "https://console.cloud.google.com/apis/credentials", label: "Open Google Cloud" },
  ...
  async function connectGoogle(provider: "gmail" | "calendar") {
    setConnectingGoogle(provider); setConnectionMessage("");
    try {
      const response = await fetch(`/api/integrations/${provider}/start`, { method: "POST" });
      const body = await readApiResponse(response, `Could not start ${provider === "gmail" ? "Gmail" : "Google Calendar"} authorization.`);
      if (typeof body.url !== "string") throw new Error("Google authorization returned no destination.");
      window.location.assign(body.url);
    } catch (caught) { setConnectionMessage(caught instanceof Error ? caught.message : "Could not start Google authorization."); setConnectingGoogle(null); }
  }
  ...
  return <>
    ...
    <div className="settings-grid">
      ...
      {cards.map(({ key, label, action }) => { const saved = integrations.find((item: any) => item.id === key); const live = health.integrations?.[key] || { status: "connection_required", checkedAt: null, configured: false }; const capabilities = key === "meta" ? "Facebook Page and Instagram professional account · read-only validation" : saved?.capabilities || "Connection adapter is not configured."; const googleProvider = key === "gmail" ? "gmail" : key === "scheduling" ? "calendar" : null; return <article className="setting-card" key={key}><div><small>{label}</small><h3>{statusLabel(live.status)}</h3><p>{live.message || capabilities}</p><span>{capabilities}</span>{live.callbackUrl && <span>OAuth callback: {live.callbackUrl}</span>}<span>Last successful check: {live.status === "ready" && live.checkedAt ? new Date(live.checkedAt).toLocaleString() : "Never"}</span></div><div className="setting-actions"><Status kind={live.status} text={statusLabel(live.status)} />{live.status !== "ready" && (key === "shopify" ? <button onClick={connectShopify} disabled={connecting}>{connecting ? "Connecting…" : action}</button> : key === "meta" && live.configured ? <button type="button" onClick={connectMeta} disabled={connectingMeta}>{connectingMeta ? "Connecting…" : action}</button> : key === "ga4" && live.configured ? <button type="button" onClick={connectGa4}>{action}</button> : googleProvider && live.configured ? <button type="button" onClick={() => connectGoogle(googleProvider)} disabled={connectingGoogle === googleProvider}>{connectingGoogle === googleProvider ? "Connecting…" : action.replace("Set up", "Connect")}</button> : <button type="button" onClick={() => openSetup(key)}>{action}</button>)}</div></article>; })}
