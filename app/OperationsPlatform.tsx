...
type Data = { tasks: any[]; memories: any[]; approvals: any[]; integrations: any[]; activity: any[]; conversations: any[]; attachments: any[] };
...
const empty: Data = { tasks: [], memories: [], approvals: [], integrations: [], activity: [], conversations: [], attachments: [] };
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {loading ? <div className="loading-state"><span></span><p>Loading verified operations…</p></div> : <section className="content">
        ...
        {view === "memory" && <Memory memories={data.memories} attachments={data.attachments} onChange={refresh} />}
...
function MediaPanel({ contextType, contextId, attachments, onChange, compact = false }: any) {
  ...
  return <div className={`media-panel ${compact ? "compact" : ""}`}><div className="media-actions"><strong>Media</strong><label className={busy ? "disabled" : ""}><input type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime" onChange={upload} disabled={busy} /><span>{busy ? "Uploading…" : "Upload image or video"}</span></label></div>{error && <p className="form-error" role="alert">{error}</p>}{items.length > 0 && <div className="media-grid">{items.map((item: any) => <a key={item.id} href={`/api/media?id=${encodeURIComponent(item.id)}`} target="_blank" rel="noreferrer" className="media-item">{item.mime_type.startsWith("image/") ? <img src={`/api/media?id=${encodeURIComponent(item.id)}`} alt={item.file_name} loading="lazy" /> : <video src={`/api/media?id=${encodeURIComponent(item.id)}`} controls preload="metadata" aria-label={item.file_name} />}<span>{item.file_name}</span><small>{item.source === "generated" ? "AI-created" : "Uploaded"}</small></a>)}</div>}</div>;
}
