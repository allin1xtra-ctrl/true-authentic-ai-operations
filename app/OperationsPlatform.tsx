...
function Settings({ integrations, health, notifications, setNotifications }: any) {
  const [connectionMessage, setConnectionMessage] = useState(() => {
    ...
    if (gmailResult) return gmailResult === "connected" ? "Gmail connected and verified with a live read-only check." : gmailResult === "denied" ? "Gmail authorization was cancelled." : gmailResult === "configuration" ? "Gmail credentials are incomplete on the server." : gmailResult === "permissions" ? "Google did not grant the required gmail.readonly scope." : "Gmail authorization could not be verified. Please try again.";
    if (calendarResult) return calendarResult === "connected" ? "Google Calendar connected and verified with a live read-only check." : calendarResult === "denied" ? "Google Calendar authorization was cancelled." : calendarResult === "configuration" ? "Google Calendar credentials are incomplete on the server." : calendarResult === "permissions" ? "Google did not grant the required calendar.readonly scope." : "Google Calendar authorization could not be verified. Please try again.";
  ...
  const setupDetails = {
    gmail: { title: "Connect Gmail", text: "Configure a Google OAuth web client using this exact callback: https://true-authentic-ai-operations.allin1xtra.chatgpt.site/api/integrations/gmail/callback. Only gmail.readonly is requested.", href: "https://console.cloud.google.com/apis/credentials", label: "Open Google Cloud" },
    ...
    calendar: { title: "Connect Google Calendar", text: "Configure a Google OAuth web client using this exact callback: https://true-authentic-ai-operations.allin1xtra.chatgpt.site/api/integrations/calendar/callback. Only calendar.readonly is requested.", href: "https://console.cloud.google.com/apis/credentials", label: "Open Google Cloud" },
  ...
  async function connectGoogleIntegration(kind: "gmail" | "calendar") {
    setConnectionMessage("");
    try { const response = await fetch(`/api/integrations/${kind}/start`, { method: "POST" }); const body = await readApiResponse(response, `Could not start ${kind === "gmail" ? "Gmail" : "Google Calendar"} authorization.`); if (typeof body.url !== "string") throw new Error("Google authorization returned no destination."); window.location.assign(body.url); }
    catch (caught) { setConnectionMessage(caught instanceof Error ? caught.message : "Google authorization could not be started."); }
  }
